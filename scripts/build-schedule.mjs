...
// dies per servei
const dow={}; if(exists('calendar.txt')) for(const c of readCSV('calendar.txt')){let m=0;
...
for await (const line of rl){
  ...
  const tr=trips[line.slice(0,c)]; if(!tr)continue;
  const f=line.split(',');
  const name=stopName[f[3]]; if(!name)continue;
  const code=nameToCode[name]; if(code==null){unmatched.add(name);continue;}
  out.push({line:tr.line,station_code:code,sentit:tr.sentit,desti:tr.desti,depart_sec:toSec(f[2]||f[1]),dow_mask:tr.mask,trip:line.slice(0,c)});
}
rl.close();
